def bwt_trancform(text: str) -> str:
  if not isinstance(text, str):
    raise TypeError("Input must be a string.")
    if len(text) < 1 or len(text) > 1000:
      raise ValueError("Input length must between 1 and 1000.")
      if text.count("$") != 1 or not text.endswith("$"):
        raise ValueError("Input must coutain exactly one '$' at the end.")
        if any(c not in "ACGT$" for c in text):
          raise ValueError("Input may contain only A,C,G,T and '$'.")

rotations = [text[:i] for i in range (len(text))]
rotations.sort()
